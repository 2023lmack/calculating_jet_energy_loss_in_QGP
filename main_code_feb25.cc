...
static inline double deltaR2(const fastjet::PseudoJet& a,
  ...
  // keep in mind we used rapiditiy instead of pseudorapidity
...
static inline bool photonIsIsolatedCone03(const Pythia8::Event& evt,
  ...
  // Build a fastjet object for the photon direction
...
double run_events(int nEvents, double ptMin, double ptMax,
  ...
  for (int iEvt = 0; iEvt < nEvents; ++iEvt) {
    ...
    if (photonTagging == true) {
      ...
      // optional: sort jets are already sorted by pt
      const double dPhiCut = 0.3; // require |Δφ| > π - 0.3  (tune as needed)

      for (int phoIdx : photonCandidates) {
        const auto& pho = pythia.event[phoIdx];

        // isolation requirement: sum pT of non-photons in ΔR<0.3 < 3 GeV
        if (!photonIsIsolatedCone03(pythia.event, phoIdx, particles, 0.3, 3.0)) continue;


        // (optional) photon acceptance checks here if you want minimal:
        // if (std::abs(pho.eta()) > 2.5) continue;

        int bestJetIdx = -1;
        double bestScore = 1e9; // smaller is better where score = |π - |Δφ|| 
        ...
        for (int jIdx = 0; jIdx < (int)jets.size(); ++jIdx) {
          ...
          double score = std::abs(M_PI - dphi);   // 0 means perfectly back-to-back
        ...
        // IMPORTANT: decide whether you want multiple photons to contribute per event.
        // If NO (most common): keep only the highest-pT photon => break after first photon.
        // If YES: keep going and potentially fill multiple times per event.
      }
    }
